...
[[ `type -P xml2` ]] || echo "Le programme xml2 n'est pas disponible.";
...
sleep 1
unzip -DD -qq "$IPA" -d tmp/ 
IPASubDirName=$(basename `find tmp/Payload/ -maxdepth 1 | tail -n1`)
echo "Sous-dossier : "$IPASubDirName
sleep 1

#2015-09-21 : Xcode 7 envoie ses plist en texte et non plus en .plist binaire.
file "tmp/Payload/$IPASubDirName/Info.plist" | grep -i text > /dev/null
IsText=$?
...
BUNDLE_IDENTIFIER=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 CFBundleIdentifier | tail -n1 | cut -d '=' -f 2`
#echo $BUNDLE_IDENTIFIER
BUNDLE_VERSION=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 CFBundleVersion | tail -n1 | cut -d '=' -f 2`
BUNDLE_SHORTVERSION=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 CFBundleShortVersionString | tail -n1 | cut -d '=' -f 2`
BUNDLE_TITLE=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 CFBundleDisplayName | tail -n1 | cut -d '=' -f 2`
#echo $BUNDLE_TITLE
BUNDLE_NAME=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 CFBundleName | tail -n1 | cut -d '=' -f 2`
#echo $BUNDLE_NAME
MINIMUM_OSVERSION=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 MinimumOSVersion | tail -n1 | cut -d '=' -f 2`
ICON_FILENAME=`$ParseInfo "tmp/Payload/$IPASubDirName/Info.plist" | xml2 | grep -A 1 CFBundleIconFile | tail -n1 | cut -d '=' -f 2`
...
cat "tmp/Payload/$IPASubDirName/embedded.mobileprovision" | php -r '$C=file_get_contents("php://stdin"); $C=substr($C, strpos($C, "<?xml")); $C=substr($C, 0, strpos($C, "</plist>")+8); echo "$C\n";' > tmp/embedded.mobileprovision.plist
